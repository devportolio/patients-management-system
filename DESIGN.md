...
This is an application (not a marketing site), so the system is applied pragmatically:

- **Dark is the default and primary theme**, mapped 1:1 to the tokens above (canvas `#010102`, the four-step surface ladder, hairline borders, `#f7f8f8` ink, lavender `#5e6ad2`). A restrained light theme is retained for the theme toggle (the brief asked for light + optional dark) using the same lavender accent.
- **Lavender is scarce** — brand mark, the primary "Add patient" CTA, focus rings, and links only. Never a card fill or background.
- **Hierarchy via the surface ladder + hairlines**, not shadows. Cards sit on `surface-1` with a 1px hairline; hovered rows and the popover/dropdown surfaces step up the ladder.
- **Radii** follow the scale: buttons & inputs `8px` (`rounded-md`), cards & dialogs `12px` (`rounded-lg`), the brand tile `16px` (`rounded-xl`). CTAs are never pill-rounded.
- **Type** uses **Inter** (the documented substitute) with aggressive negative tracking on headings and weight 600 display / 400 body.
- **No atmospheric gradients or spotlight cards** — the login screen was deliberately stripped of its glow/gradient to honor this.
- Beyond the single success green, the app keeps a muted `destructive` red and `warning` amber because a CRUD tool genuinely needs them (delete confirmation, etc.) — a conscious, documented departure from the marketing-only "one semantic color" rule.

Token source of truth: `apps/web/src/app/globals.css` (CSS variables) and `apps/web/tailwind.config.ts` (scale mapping).
